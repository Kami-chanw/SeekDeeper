...
class Block(nn.Module):
    def __init__(self, d_model, n_head, dropout, device):
        ...
        self.attn = nn.MultiheadAttention(
            d_model, num_heads=n_head, dropout=dropout, device=device
        )
        self.ln_2 = nn.LayerNorm(d_model, device=device)
        self.mlp = nn.ModuleDict(
            dict(
                c_fc=nn.Linear(d_model, 4 * d_model, device=device),
                c_proj=nn.Linear(4 * d_model, d_model, device=device),
                act=nn.GELU(),
    ...
    def forward(self, x):
        x = self.ln_1(x)
        x = x + self.attn(x, x, x, is_causel=True)[0]
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        params_given = all(
            [config.n_layer is not None, config.n_head is not None, d_model is not None]
        )
