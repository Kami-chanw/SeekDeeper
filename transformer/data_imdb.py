...
class Tokenizer:
    ...
    def encode(self, text):
        ...
        token_ids = [self.vocab[CLS_TOKEN]] + [self.vocab.get(word, self.vocab[UNK_TOKEN]) for word in tokens]
...
class IMDBDataset(Dataset):
    def __init__(self, data_directory, tokenizer, label_type):
        ...
        file_list = [os.path.join(dir_name, fname) for fname in os.listdir(dir_name) if fname.endswith('.txt')]

        with ThreadPoolExecutor() as executor:
            results = list(tqdm(executor.map(lambda f: read_file(f, tokenizer, label_type), file_list), total=len(file_list), desc=f"Loading {label_type} data"))
...
def collate_batch(batch):
    ...
    data = torch.nn.utils.rnn.pad_sequence(data, batch_first=True, padding_value=vocab[PAD_TOKEN])
